...
class SupportFluentTest extends TestCase
{
    ...
    {
        $fluent = new Fluent(['name' => 'Frankie']);

        $this->assertEquals('Frankie', $fluent->get('name'));
        $this->assertEquals('Default', $fluent->get('foo', 'Default'));
        $this->assertEquals('Frankie', $fluent->name);
        $this->assertNull($fluent->foo);

        $fluent = new Fluent(['profile' => ['name' => 'Frankie']]);

        $this->assertEquals('Frankie', $fluent->get('profile.name'));
